    ...
    function drawSnake() {
        for (let i = 0; i < snake.length; i++) {
            ...
            if (i == 0) {
                ...
                // Check if this segment is turning
                const turning =
                    (prev.direction !== curr.direction && next.direction !== curr.direction);

                if (turning) {
                    graphics.drawImage(turn, curr.x, curr.y, blockwidth, blockheight);
                } else {
                    // Straight body (LR or UD)
                    if (curr.direction == 'left' || curr.direction == 'right') {
                        graphics.drawImage(LRbody, curr.x, curr.y, blockwidth, blockheight);
                    } else {
                        graphics.drawImage(UDbody, curr.x, curr.y, blockwidth, blockheight);
                    }
                }
            }
        }
